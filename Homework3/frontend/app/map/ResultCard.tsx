...
export default function ResultCard({
  ...
  const handleReviews = async () => {
    ...
    const reviewsFetch = await axios.get(`http://localhost:8080/map-locations/${mid}/reviews`,
      {
        headers: { Authorization: `Bearer ${authToken}` },
      }
    );
  ...
  const handleAddReview = async () => {
    ...
    await axios.post(`http://localhost:8080/map-locations/${mid}/addReview`,
      {
  ...
  const handleDeleteReview = async (reviewId: number) => {
    await axios.delete(`http://localhost:8080/map-locations/${mid}/deleteReview/${reviewId}`,
      {
        headers: { Authorization: `Bearer ${authToken}` }
      }
    );
  ...
  const handleEditSubmit = async () => {
    await axios.put(`http://localhost:8080/map-locations/${mid}`,
      {
        ...
        <div className="text-lg">
          ...
          {editMode && 
            <div className="inline">
              ...
              />
              <button onClick={handleEditSubmit} className="ml-2 bg-slate-700 text-white text-sm p-1 h-9 rounded">Submit</button>
            </div>
          }
          {roles.includes("ADMIN") && <small onClick={() => handleEdit(title as string)} className="ml-2 underline cursor-pointer">Edit</small>}
        </div>
            ...
            <div className="my-2" key={rev.id}>
              ...
              <small>
                ...
                {localStorage.getItem("username") === rev.postedBy && 
                  <span onClick={() => handleDeleteReview(rev.id)} className="ml-5 underline cursor-pointer">Delete</span>
                }
              </small>
            </div>
          ))}
